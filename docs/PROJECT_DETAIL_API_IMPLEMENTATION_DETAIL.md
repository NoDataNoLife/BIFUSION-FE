...
updateProjectInfo: async (projectId: string, data: any) => {
  ...
  try {
    ...
    if (response.data.success) {
      // 수정 성공 시 기존 상태에 payload 값만 병합(merge)하여 화면을 갱신합니다.
      set((state) => ({
...
**핵심 포인트**: API 응답은 `{ success: true, data: { ... } }` 형태로 오기 때문에 `response.data.data`에 접근해야 하며, 수정을 완료한 뒤에는 API 재호출 없이 기존 상태 값만 전개 연산자(`...`)로 부분 업데이트하여 화면을 효율적으로 갱신합니다.
...
3. **상태 업데이트** 👉 백엔드 데이터(`response.data.data`)가 `currentProject`에 저장됨
4. **화면 갱신** 👉 `currentProject` 데이터를 활용해 제목/설명 렌더링 및 모달 폼 초기화
5. **데이터 수정** 👉 사용자가 모달에서 값을 변경하고 `handleSaveSettings` 실행
6. **수정 반영** 👉 `updateProjectInfo`가 API를 호출한 뒤, 성공 시 전개 연산자를 활용해 `currentProject` 상태를 부분 업데이트함
