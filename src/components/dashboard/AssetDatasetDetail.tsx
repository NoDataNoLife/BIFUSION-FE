...
import { ArrowLeft, Database, Download, HardDrive, FileText, Calendar, Code, Info, Trash2, Edit, X, Tag, Clock, Play, Award, CheckCircle, AlertCircle } from 'lucide-react';
...
interface AssetDatasetDetailProps {
  ...
  onBack: () => void;
  onDelete: () => void;
  onUpdate?: (updatedData: { name: string; description: string; tags: string[] }) => void;
}
...
export default function AssetDatasetDetail({ dataset, onBack, onDelete, onUpdate }: AssetDatasetDetailProps) {
  const [isDownloading, setIsDownloading] = useState(false);
  const [selectedFiles, setSelectedFiles] = useState<string[]>([]);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [newName, setNewName] = useState(dataset.name);
  const [newDescription, setNewDescription] = useState(dataset.description || '');
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-8 py-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setIsEditing(true)} className="p-2.5 text-gray-400 hover:bg-gray-50 rounded-xl transition-all"><Edit size={20} /></button>
            <button onClick={() => setShowDeleteConfirm(true)} className="p-2.5 text-red-400 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={20} /></button>
            <button onClick={handleDownload} disabled={isDownloading} className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all text-sm disabled:opacity-50">
              <Download size={18} /> {isDownloading ? '준비 중...' : '전체 다운로드'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-10 space-y-8">
        ...
        <div className="bg-white rounded-[2rem] p-10 border border-gray-100 shadow-sm flex flex-col md:flex-row gap-10">
          <div className="w-full md:w-64 h-64 rounded-3xl overflow-hidden shadow-inner flex-shrink-0">
            <img src={dataset.thumbnail} alt={dataset.name} className="w-full h-full object-cover" />
          </div>
          
          <div className="flex-1 space-y-6">
            <div className="flex items-center gap-3">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${dataset.type === 'augmented' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>
                {dataset.type}
              </span>
            ...
            <p className="text-gray-500 font-medium leading-relaxed max-w-2xl">
              {dataset.description || (dataset.type === 'augmented' 
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-6 border-t border-gray-50">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">총 용량</p>
                <p className="font-bold text-gray-900">{dataset.size}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">파일 수</p>
                <p className="font-bold text-gray-900">{dataset.fileCount.toLocaleString()}개</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">업로드 날짜</p>
                <p className="font-bold text-gray-900">{dataset.uploadedAt}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">데이터 형식</p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-900 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden">
              ...
              <h3 className="text-xl font-black text-white mb-6 flex items-center gap-3">
                <Terminal size={20} className="text-primary" /> Python API 사용 예시
              </h3>
          ...
          <div className="space-y-8">
                ...
                {dataset.type === 'augmented' && (
                  <div className="pt-4 mt-4 border-t border-gray-50 space-y-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold text-primary">Diffusion v4</span>
                    </div>
                    <div className="flex justify-between text-sm">
            ...
            <div className="bg-primary rounded-[2rem] p-8 text-white shadow-xl shadow-primary/20">
              ...
              <p className="text-white/80 text-sm font-medium leading-relaxed mb-6">
                품질이 검증된 데이터셋은 연구 신뢰도를 높여줍니다. 전문가에게 검증을 요청하세요.
              </p>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full">
            <h2 className="text-2xl font-black text-gray-900 mb-2">데이터셋 삭제</h2>
            <p className="text-gray-500 font-medium mb-8">"{dataset.name}"을(를) 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.</p>
            <div className="flex gap-4">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-4 bg-gray-100 text-gray-500 rounded-2xl font-bold">취소</button>
              <button onClick={handleDeleteConfirm} className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-bold">삭제하기</button>
            </div>
          </div>
        </div>
      )}
