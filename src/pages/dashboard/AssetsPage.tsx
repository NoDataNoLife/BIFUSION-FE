...
export default function AssetsPage() {
  ...
  return (
    <div className="p-8 space-y-10 max-w-7xl mx-auto">
      ...
      {/* Navigation Tabs */}
      <div className="flex gap-2 p-1 bg-white border border-gray-100 rounded-[1.25rem] w-fit shadow-sm">
        {(['recipes', 'uploaded-recipes', 'datasets', 'augmented'] as const).map(tab => (
          <button
            ...
            className={`px-6 py-2.5 rounded-2xl text-sm font-black transition-all ${
              activeTab === tab ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'
            }`}
          >
            ...
            >
              <div className="aspect-video relative overflow-hidden bg-gray-50 p-4 flex items-center justify-center">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <button className="p-3 bg-white rounded-xl hover:bg-primary hover:text-white transition-all text-gray-900"><Download size={20} /></button>
                  <button className="p-3 bg-white rounded-xl hover:bg-red-500 hover:text-white transition-all text-red-500"><Trash2 size={20} /></button>
                </div>
              </div>
              <div className="p-8">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-black text-gray-900 line-clamp-1 flex-1">{dataset.name}</h3>
                  <button className="p-1 hover:bg-gray-50 rounded-lg"><MoreVertical size={16} className="text-gray-300" /></button>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Size</p>
                    <p className="text-sm font-bold text-gray-900">{dataset.size}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Files</p>
                    <p className="text-sm font-bold text-gray-900">{dataset.fileCount.toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                  ...
                  <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">{dataset.format}</span>
                </div>
              </div>
            </div>
          ))
