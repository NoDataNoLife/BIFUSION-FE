  ...
  return (
    <div className="min-h-screen bg-gray-50/50 pb-20">
        ...
        <div className="max-w-6xl mx-auto px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-gray-900 tracking-tight italic flex items-center gap-2">
                추론 완료 리포트 <CheckCircle2 className="w-6 h-6 text-green-500" />
              </h1>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-8 py-8 space-y-10">
        ...
        {/* Charts Section (Prototype Visuals) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm space-y-6">
            ...
            <div className="aspect-video bg-gray-50 rounded-3xl border border-gray-100 flex items-center justify-center text-gray-300 italic font-bold">
              [클래스 비율 파이 차트 시각화]
            </div>
          </div>
          <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm space-y-6">
        ...
        {/* Confidence Filter (Prototype Slider) */}
        <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm space-y-8">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-black text-gray-900 tracking-tight">결과 필터링</h3>
            <div className="px-4 py-2 bg-primary/5 text-primary rounded-xl font-black text-xs uppercase tracking-widest">
        ...
        {/* Results Table (Prototype structure) */}
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">이미지</th>
                <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">예측 클래스</th>
                <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">Confidence</th>
                <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">실제 라벨</th>
                <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest text-right">Top-3 Support</th>
              </tr>
            </thead>
              ...
              {mockResults.map((item) => (
                <tr key={item.id} className="group hover:bg-gray-50/50 transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-gray-100 rounded-2xl border border-gray-200 group-hover:scale-110 transition-transform shadow-sm" />
                      <span className="font-bold text-gray-700 text-sm">{item.filename}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
                      item.predicted === 'Normal' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'
                    }`}>
                      {item.predicted}
                    </span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-black text-gray-900 italic tabular-nums">{item.confidence}%</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className={`text-sm font-black ${item.correct ? 'text-green-600' : 'text-red-500'} italic`}>
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                      {[1, 2, 3].map(i => <div key={i} className="w-8 h-8 bg-gray-200 rounded-lg" />)}
                    </div>
                  </td>
                </tr>
              ))}
            ...
            {comments.map((c) => (
              <div key={c.id} className="flex gap-5 group">
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="font-black text-gray-900 text-sm">{c.user}</span>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{c.time}</span>
                    </div>
                    <button className="opacity-0 group-hover:opacity-100 p-1 text-gray-400 hover:text-gray-900 transition-all">
                      <MoreVertical className="w-4 h-4" />
                    </button>
                  </div>
                  <div className="p-6 bg-gray-50 rounded-3xl rounded-tl-none border border-gray-100 text-gray-600 font-medium leading-relaxed">
                    {c.content}
                  </div>
                </div>
              </div>
