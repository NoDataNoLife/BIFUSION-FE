      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-8 py-6 flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">모델 추론 설정</h1>
          </div>
        </div>
      </div>

      <div className="max-w-4xl mx-auto px-8 py-8">
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-12 space-y-10">
          <p className="text-gray-500 font-medium italic">학습된 모델로 새로운 이미지를 분류합니다</p>
          ...
          <div className="space-y-4">
            ...
            >
              <div className="flex flex-col items-center justify-center">
                <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center shadow-sm mb-6 group-hover:scale-110 transition-transform">
                  <FileImage className={`w-10 h-10 ${uploadedFiles.length > 0 ? 'text-primary' : 'text-gray-400'}`} />
                </div>
                <p className="text-xl font-black text-gray-900 mb-2 tracking-tight">파일 선택 또는 드래그 & 드롭</p>
                <p className="text-sm text-gray-400 font-bold uppercase tracking-widest mb-6">JPG, PNG (배치 업로드 가능)</p>
            ...
            {uploadedFiles.length > 0 && (
              <div className="mt-6 p-6 bg-gray-50 rounded-3xl border border-gray-100 space-y-3">
                <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">
                  업로드된 파일: <span className="text-primary">{uploadedFiles.length}개</span>
                </p>
                <div className="max-h-40 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                  {uploadedFiles.slice(0, 5).map((file, index) => (
                    <div key={index} className="flex items-center justify-between text-sm font-bold text-gray-700 bg-white rounded-xl p-3 shadow-sm border border-gray-100/50">
                      ...
                      <span className="text-[10px] text-gray-400 uppercase tabular-nums">{(file.size / 1024).toFixed(1)} KB</span>
                    </div>
                  ))}
                  {uploadedFiles.length > 5 && (
                    <p className="text-center text-[10px] font-black text-gray-300 uppercase tracking-tighter pt-2">...외 {uploadedFiles.length - 5}개의 파일이 더 있습니다</p>
                  )}
                </div>
              </div>
            )}
