          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-foreground tracking-tight italic flex items-center gap-2">
                학습 완료 리포트 <CheckCircle2 className="w-6 h-6 text-emerald-500" />
              </h1>
            </div>
          </div>

          <div className="ml-auto flex items-center gap-3">
            <button className="flex items-center gap-2 px-5 py-2.5 bg-muted text-muted-foreground hover:text-foreground border border-border rounded-2xl font-black text-sm transition-all shadow-sm cursor-pointer">
              <Download size={18} />
              모델(PTH) 다운로드
            </button>
            <button 
              onClick={() => navigate(`/dashboard/projects/${projectId}/inference/new-job/setup`)}
              className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-2xl font-black text-sm hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 cursor-pointer"
            >
        ...
        <div className="bg-card rounded-4xl p-10 border border-border shadow-sm space-y-8">
          ...
          <div className="relative">
            ...
            <button 
              ...
              className={`absolute bottom-4 right-4 p-4 rounded-2xl transition-all shadow-lg cursor-pointer ${
          ...
          <div className="space-y-6 pt-4">
            {comments.map((comment) => (
              <div key={comment.id} className="flex gap-4 group">
                <img src={comment.avatar} alt={comment.user} className="w-12 h-12 rounded-2xl bg-muted ring-2 ring-border shrink-0" />
                <div className="flex-1 space-y-2">
                  ...
                  <div className="p-5 bg-muted/40 rounded-3xl rounded-tl-none border border-border text-muted-foreground font-medium leading-relaxed">
                    {comment.content}
                  </div>
                </div>
              </div>
